...
def plot_hagrid_accuracy_profile(df):
    ...
    lower, upper = focused_limits(all_values, pad=3, min_span=18)
    ax.set_ylim(lower, upper)
    ax.set_xlim(-0.35, len(reps) - 1 + 1.2)
    ...
    fig.subplots_adjust(left=0.10, right=0.84, bottom=0.23, top=0.96)
...
def plot_robustness_gap(df):
    ...
    max_val = np.nanmax(pivot.values.astype(float))
    ax.set_xlim(0, max_val + 3.2)
...
def plot_rank_summary(df):
    rank_df = df.copy()
    rank_df["condition"] = rank_df["dataset"].astype(str) + " / " + rank_df["classifier"].astype(str)
    ...
    labels = [REP_DISPLAY.get(str(rep), str(rep)) for rep in mean_rank["representation"]]
...
def plot_controlled_vs_hagrid_slope(df):
    ...
    for _, row in slope_df.iterrows():
        ...
        color = color_map.get(classifier, LOGISTIC_COLOR)
    ...
    for classifier in CLASSIFIER_ORDER:
        if classifier in slope_df["classifier"].values:
            ...
            labels.append(CLASSIFIER_DISPLAY[classifier])
    ...
    fig.subplots_adjust(left=0.18, right=0.96, bottom=0.28, top=0.96)
