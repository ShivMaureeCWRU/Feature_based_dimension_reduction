...
CLASSIFIER_NAME = "Multinomial logistic"
...
FEATURE_FUNCTIONS = {
    ...
    "Scaled": phi_scaled,
    "Distance": phi_pairwise_distance,
...
def build_level_config(perturbation_type, level):
    base = BASE_LEVELS[level]

    if perturbation_type == "Translation only":
        return {
    ...
    if perturbation_type == "Scale only":
        return {
            "translation": 0.00,
            "scale_low": base["scale_low"],
            "scale_high": base["scale_high"],
            "noise": 0.000,
        }

    if perturbation_type == "Translation + scale":
        return {
            "translation": base["translation"],
            "scale_low": base["scale_low"],
            "scale_high": base["scale_high"],
    ...
    if perturbation_type == "Translation + scale + noise":
        return {
            "translation": base["translation"],
            "scale_low": base["scale_low"],
            "scale_high": base["scale_high"],
...
def run_experiment():
    print("\n=== Perturbation-type robustness experiment ===")
    ...
    for rep in REPRESENTATIONS:
        ...
        X_test_clean = FEATURE_FUNCTIONS[rep](L_test)

        model, best_params, best_cv = fit_logistic_model(X_train, y_train)

        y_pred_clean = model.predict(X_test_clean)
        clean_acc = accuracy_score(y_test, y_pred_clean) * 100.0

        trained_models[rep] = model
        clean_accuracy_by_rep[rep] = clean_acc

        clean_baselines.append(
            {
                "representation": rep,
                "classifier": CLASSIFIER_NAME,
                "clean_test_accuracy": clean_acc,
                "best_cv_accuracy": best_cv * 100.0,
    ...
    clean_baseline_df = pd.DataFrame(clean_baselines)

    clean_path = OUTPUT_DIR / "perturbation_type_clean_baselines.csv"
    clean_baseline_df.to_csv(clean_path, index=False)
    print(f"\nSaved clean baselines to: {clean_path}")


    rows = []

    for type_index, perturbation_type in enumerate(PERTURBATION_TYPES):
        print(f"\n--- {perturbation_type} ---")

        for level in BASE_LEVELS.keys():
            config = build_level_config(perturbation_type, level)
            ...
            for repeat in range(PERTURBATION_REPEATS):
                ...
                for rep in REPRESENTATIONS:
                    ...
                    model = trained_models[rep]
                    y_pred = model.predict(X_test)

                    acc = accuracy_score(y_test, y_pred) * 100.0
                    clean_acc = clean_accuracy_by_rep[rep]
                    drop = clean_acc - acc
    ...
    raw_path = OUTPUT_DIR / "perturbation_type_results.csv"
    ...
    summary = (
        ...
        .agg(
            ...
            min_accuracy_drop=("accuracy_drop", "min"),
    ...
    return results, summary, clean_baseline_df
