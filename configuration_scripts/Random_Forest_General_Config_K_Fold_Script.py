...
OUTPUT_BASE_DIR="../random_forest_outputs"
MODEL_BASE_DIR="../models/random_forest_models"
...
SUMMARY_LOG_PATH=os.path.join(
    OUTPUT_BASE_DIR,
    "random_forest_console_summary.txt"
)
...
for representation_name,file_name in representation_files.items():
    ...
    summary_df.to_csv(
        os.path.join(OUTPUT_DIR,"random_forest_summary.csv"),
        index=False
    )
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(OUTPUT_DIR,"random_forest_confusion_matrix_plot.png"),
        dpi=300
    )
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(OUTPUT_DIR,"random_forest_accuracy_plot.png"),
        dpi=300
    )
    plt.close()
    ...
    dump(
        random_forest_model,
        os.path.join(MODEL_DIR,f"random_forest_{representation_name}.joblib")
    )
...
all_results_df.to_csv(
    os.path.join(OUTPUT_BASE_DIR,"random_forest_all_representations_summary.csv"),
    index=False
)
...
if len(all_results_df)>0:
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(OUTPUT_BASE_DIR,"random_forest_representation_test_accuracy.png"),
        dpi=300
    )
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(OUTPUT_BASE_DIR,"random_forest_representation_cv_accuracy.png"),
        dpi=300
    )
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(OUTPUT_BASE_DIR,"random_forest_all_representations_summary_plot.png"),
        dpi=300,
        bbox_inches="tight"
    )
    plt.close()
